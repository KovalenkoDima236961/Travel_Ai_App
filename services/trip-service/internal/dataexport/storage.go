// Package dataexport contains the small private-file primitives shared by the
// trip export lifecycle. Files are intentionally addressed only by an opaque
// server-generated key; callers never receive a storage path.
package dataexport
...
// Save persists an already-generated private package. The key is generated by
// the service, never from a title or user-provided filename.
func (s *LocalStorage) Save(key string, content []byte) (string, int64, string, error) {
	...
	return key, int64(len(content)), hex.EncodeToString(sum[:]), nil
}
