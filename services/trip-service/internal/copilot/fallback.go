...
func fallbackResponse(intent Intent, context SafeContext, available []Action, language string) AIResponse {
	...
	case IntentExplainRecap:
		if recap := context.Recap; recap != nil {
			answer = fmt.Sprintf(fallbackText(language, "recap"), recap["doneItemCount"], recap["plannedItemCount"], recap["lessonCount"])
		} else {
			answer = fallbackText(language, "recap_missing")
		}
		sources = []string{"trip_recap"}
