...
func (c *closer) closeAll(ctx context.Context) error {
	...
	c.once.Do(func() {
		...
		slog.Info("starting finishing", "count", len(funcs))
		...
		for i := len(funcs) - 1; i >= 0; i-- {
			...
			if err := f.fn(ctx); err != nil {
				slog.Error("error during closing resource",
					"name", f.name,
					"error", err,
				...
				slog.Info("resource is closed", "name", f.name, "duration", time.Since(start))
			}
		}

		slog.Info("all resources are closed")
