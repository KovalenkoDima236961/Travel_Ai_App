...
AI model-serving rollout decisions are also owned by Trip Service. Worker
generation paths reuse the same Trip Service router composition, forward only
safe deployment assignment metadata to AI Planning Service, and keep RabbitMQ
messages free of prompts, itinerary JSON, adapter paths, and provider payloads.
Shadow-evaluation queue contracts are defined for rollout playbooks, but
candidate promotion/rollback state changes remain Trip Service operational
actions.
