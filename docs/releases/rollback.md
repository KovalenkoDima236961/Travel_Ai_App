...
| AI model rollout | Candidate quality regression, latency spike, guardrail breach, or shadow queue pressure. | Pause or roll back the model deployment with `scripts/ai/pause-model-deployment.sh` or `scripts/ai/rollback-ai-model.sh`, keep the additive tables, and verify baseline generation plus online metrics. | Do not drop serving tables or delete comparison rows during incident response; preserve evidence and use a forward fix for bad persisted output. |
| Closed alpha | Alpha smoke, public share, OpenAI safeguards, or Ops recovery fails after invite. | Pause invites, disable the narrowest feature flag, create/verify backup, run `scripts/release/rehearse-alpha-rollback.sh`, then verify alpha smoke and versions. | Do not run down migrations automatically; mark rollback blocked when old images cannot read the current schema. |
...
Closed alpha rollback details live in [alpha rollback](../operations/runbooks/alpha-rollback.md) and the generated `dist/alpha-readiness/rollback-rehearsal-report.md`.
