...
import type { RemoveRouteLegTransportOptionInput } from "@/types/transport";

export function useRemoveRouteLegTransportOption(tripId: string, legId: string) {
  const queryClient = useQueryClient();
  return useMutation<Trip, Error, RemoveRouteLegTransportOptionInput | undefined>({
    mutationFn: (input) => removeRouteLegTransportOption(tripId, legId, input ?? {}),
