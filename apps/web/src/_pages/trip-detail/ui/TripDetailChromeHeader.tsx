...
/**
 * Redesigned app chrome for the Trip Detail screen. It ships with the slice
 * (AppHeader is suppressed for the exact `/trips/[id]` route) so the warm palette
 * does not leak onto the app pages that still render the old slate header. Like
 * CreateTripHeader, it drops the Trips scope select and the "New trip" CTA.
 */
export function TripDetailChromeHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-sand-300 bg-sand-50/95 backdrop-blur">
      <div className="mx-auto flex max-w-[1360px] items-center justify-between gap-3 px-4 py-2 sm:gap-6 sm:px-10 sm:py-3">
