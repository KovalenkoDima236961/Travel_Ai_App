  ...
  return (
    <Card className={className}>
      ...
      {filteredMarkers.length === 0 ? (
        ...
        <div className="mt-5 h-[260px] overflow-hidden rounded-lg border border-slate-200 bg-slate-100 sm:h-[420px]">
          <LeafletItineraryMap
            center={center}
            currency={currency}
...
function FilterButton({ label, selected, onClick }: FilterButtonProps) {
  return (
    <button
      ...
      className={cn(
        "min-h-11 rounded-md border px-3 py-2 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2",
        selected
