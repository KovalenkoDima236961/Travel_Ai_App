...
ITERATIONS="${PERF_SMOKE_ITERATIONS:-20}"
P95_THRESHOLD_MS="${PERF_SMOKE_P95_THRESHOLD_MS:-1000}"
...
if [[ -z "${TRIP_ID}" ]]; then
  echo "No trip found. Set PERF_TRIP_ID to a seeded, feature-rich trip." >&2
  exit 1
fi

endpoints=(
  "trip_detail|/trips/${TRIP_ID}"
  "command_center|/trips/${TRIP_ID}/command-center-summary"
  "health|/trips/${TRIP_ID}/health"
  "budget_confidence|/trips/${TRIP_ID}/budget-confidence"
  "group_readiness|/trips/${TRIP_ID}/group-readiness?includeDetails=true"
  "activity|/trips/${TRIP_ID}/activity?limit=30"
  "expense_summary|/trips/${TRIP_ID}/expenses/summary"
)

failed=0
echo "Performance smoke: trip=${TRIP_ID}, iterations=${ITERATIONS}, p95 threshold=${P95_THRESHOLD_MS}ms"
for definition in "${endpoints[@]}"; do
  label="${definition%%|*}"
  path="${definition#*|}"
  timings="${PERF_TMP_DIR}/${label}.timings"
  statuses="${PERF_TMP_DIR}/${label}.statuses"
  body="${PERF_TMP_DIR}/${label}.json"
  : >"${timings}"
  : >"${statuses}"

  for ((iteration = 1; iteration <= ITERATIONS; iteration++)); do
    result="$(curl -sS -o "${body}" -w '%{http_code} %{time_total}' -H "Authorization: Bearer ${ACCESS_TOKEN}" "${TRIP_SERVICE_URL}${path}")"
    status="${result%% *}"
    seconds="${result#* }"
    ...
    if [[ "${label}" == "command_center" ]] && grep -Eqi '"(rawText|storageKey|prompt|contentRedacted|commentBody|notes)"[[:space:]]*:' "${body}"; then
      echo "command_center: forbidden private field found in response." >&2
      failed=1
    fi
  done
...
if [[ "${failed}" -ne 0 ]]; then
  echo "Performance smoke failed." >&2
  exit 1
fi
